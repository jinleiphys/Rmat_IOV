...
c wg(1:ng)= weigths
    ...
    1 continue
      ...
      subroutine rmat_inho(ng,rmax,cpot,csou,ecm,eta,hm,l,cu,cf)
...
C      crho(i)=csou(i)*sqrt(wg(i)*rmax) !  souce term function in Lagrange mesh basis < \phi_i | \rho> ! See details in pierre's notes
    1 continue
...
C      cmat=c
C      cmat_rho=crho
C      call ZGESV( ng, NRHS, cmat, ng, IPIV, cmat_rho, ng, INFO )  ! used for computed C^{-1} \rho
C      If(INFO/=0) stop "error in calling ZGESV"
C
...
!      call coufra(ak,eta,l,l,fc,dfc,gc,dgc) ! call the coulomb function;  this can be replaced !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      cu=(ci-ak*cip*cr)/(co-ak*cop*cr) ! S-matrix for homogeneous equation
C      cut=-cr2/(co-ak*cop*cr)             ! S-matrix for inhomogeneous equation

c      crho(1:ng)=crho(1:ng)-cut*xk*cop*hm*g(1:ng)
c      cft(1:ng)=matmul(c(1:ng,1:ng),crho(1:ng))
C      cft(1:ng) =  cmat_rho(1:ng) - cut*xk*cop*hm*cmat_phi(1:ng)
C      cft(1:ng)=cft(1:ng)/sqrt(rmax*wg(1:ng))  ! compute the wave function of inhomogeneous equation

      cf(1:ng)=matmul(c(1:ng,1:ng),g(1:ng))
      cf(1:ng)=cf(1:ng)*hm*xk*(ci-cu*co)            ! compute the wave function of homogeneous equation
      return
      end SUBROUTINE
